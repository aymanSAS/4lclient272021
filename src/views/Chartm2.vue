...
<div>
 ...
 <h2 class='bluestyle'> {{$store.state.machineName    }} _Current Product: {{ $store.state.sbatch[0].productcode}} </h2>
<div style="float:left;">
  <h3>     {{mydate | formatDateonly2}} </h3>

  <!-- <h3>     {{$store.state.machineName  }} </h3> -->

<h3 >Batch ID:

  {{$store.state.sbatch[0].runid}} </h3>

<img src="images/LOGOTYMANDMOBIL.png" width="300" height="310"   >
    </div>
<div class="container">
  <div class='row'>
  <div class='col-sm'>
...
<hr >
  <div class='row'>
  <div class='col-sm'>
        <h3 class='bluestyle'> Target Rate : {{myTargetRate | formatNumber}} Carton/hr</h3>

  </div>
  <div class='col-sm'>
   <h3 :class="{'redstyle': (onlinerateCarton < myTargetRate ),'greenstyle':(onlinerateCarton >myTargetRate)}"> Current Rate : {{onlinerateCarton | formatNumber}} Carton/hr</h3>
  </div>
 </div>
<hr >
...
<div class='row'>
  ...
  <!-- <div class='col-sm'>
    <h3 class='regstyle'> Monthly Production : {{PPD| formatNumber}} BBL</h3>
      <h3 class='regstyle'> Monthly Max : {{max | formatNumber}} BBL</h3>
  </div> -->
</div>
...
<div class='row'>
  <!-- <div class='col-sm'>
   ...
   <!-- <div class='col-sm'>
      <h3 class='regstyle'> Monthly Avaiable : {{available | formatNumber}} BBL</h3>
   </div> -->
</div>
</div>
  ...
  <td  :class="{'redstyle': (PEI < myTargetPei ),'greenstyle':(PEI >myTargetPei)}"  >  {{PEI *100 | formatNumber}} %  </td>
   <td class="font-class">  {{(currentbatch[0].ppdbatchcount || 0)| formatNumber}} Unit ,
        {{(currentbatch[0].ppdbatchlitre || 0)| formatNumber}} BBL
  </td>
...
getallinbatch(){
 ...
 ReportService.getrates().then(res => {
    ...
    this.onlinerate = res.data[0].onlinerate   ;
    this.onlineratebbl = (res.data[0].onlineratebbl) ;
    this.onlinerateCarton=res.data[0].onlinerate/4;
...
.redstyle {
     color:rgb(255, 0, 0);
     font-size: 30px;
      border-style:solid; border-color:#71727a; padding: 7px;
       border-width: 10px;
}
