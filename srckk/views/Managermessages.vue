...
<button style="float: left;margin-top:20; margin-right:120px" type="button" class="btn btn-success" @click.prevent= "Getallmtime(3)">-3 Month</button> -->
                 <button style="float: left;margin-top:20; margin-right:20px" type="button" class="btn btn-success" @click.prevent= "generatepdf()">Main PDF</button>
...
<downloadexcel
 ...
 <table  class="table">
    <thead class="thead-dark" >
      ...
      <th scope="col"> Other Planned Maintenance </th>
  ...
  <td>  {{EF | formatNumber}} hrs</td>
   <td>  {{RCL/BBL | formatNumber}} hrs</td>
...
doc.text('Failures Report (4L Machine): '+ this.lastMonth +' / ' + this.y,11,11) ;
...
this.lastMonth=today.getMonth()+1-m;
    ...
    this.PCL=res.data[0].PCL *this.BBL
    ...
    this.CLOSED=res.data[0].CLOSED;
    this.PCLHF1=res.data[0].PCLHF1
    this.PCLHF2=res.data[0].PCLHF2
    this.RCLHF1=res.data[0].RCLHF1
    this.RCLHF2=res.data[0].RCLHF2
    this.RCLHF3=res.data[0].RCLHF3
    this.PPD=res.data[0].PPD *this.ppdconverter;
    
    this.MR=res.data[0].MR +res.data[0].Jams////////////- res.data[0].RCLHF3  ;
...
generatepdf(){
const doc = new jsPDF()
doc.text('Performance Report (1L Machine): '+ this.lastMonth +' / ' + this.y,15,15) ;
doc.autoTable({
  head: [['Multiplier ']],
  body: [
    [(this.BBL ) ],
...
doc.autoTable({
  head: [['RCL', 'PCL', 'PEI']],
  body: [
    [numeral(this.RCLTOTAL).format("0.00") +' BBL', numeral(this.PCL ).format("0.00")+' BBL', (numeral(this.PEI *100 ).format("0.00")) +' %'],
  ],
})
 //
doc.autoTable({
  head: [['Quality-Factor', 'Max', 'Closed','Actual']],
  body: [
    [numeral(this.qf ).format("0.00")+' BBL', numeral(this.max ).format("0.00") +' BBL', numeral(this.CLOSED * this.BBL ).format("0.00")+' BBL',this.PPD+' BBL'],
  ],
})
doc.autoTable({
  head: [['Maintenance Reliability', 'External Factor', 'RCL by time','RCL Human']],
  body: [
    [numeral(this.MR ).format("0.00")+' BBL', numeral(this.EF ).format("0.00") +' BBL', numeral(this.RCL / this.BBL ).format("0.00")+' hr',numeral(this.RCLHF+' BBL').format("0.00")],
    
  ],
})
//
doc.autoTable({
  head: [['Planned Maintenance', 'Other Planned Maintenance', 'PCL Maintenace']],
  body: [
    [(numeral(this.PL).format("0.00") )+' BBL', numeral(this.OPL  ).format("0.00") +' BBL', numeral(this.PCLHF ).format("0.00")+' BBL'],
