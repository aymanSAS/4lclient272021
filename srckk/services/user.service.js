...
class UserService {
  ...
  let r2= axios.get(API_URL + "/opmessages", { headers: authHeader() }).catch(err=>{
    if (!err){
      return r2;
  ...
  async createmessage(data) {
    let rr= await axios.post(API_URL+"/createmessage",data, { headers: authHeader() }).then(response => {
      return rr
    }).catch(err => (console.log('error create message')))
  }
  //
