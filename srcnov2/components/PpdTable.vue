...
<tr v-for ="ppd in propcats" v-bind:key ="ppd.id"  @click="selectRow(ppd.id,ppd.ppd,ppd.date)"  :class="{'highlight': (ppd.id == currentcat.id )}" >
<td>
    {{ppd.id}}
    </td>
    <td>
...
export default {
  ...
  data(){
  return {
  currentcat:{id:'',ppd:'',date:'',insertdate:'',userno:''},
  mycat:'',
  currentIndex: -1,
  cat:'',
  mycates:'',
...
newsubcat() {
      ...
      this.currentcat = {id:'',ppd:'',date:'',insertdate:'',userno:''};
